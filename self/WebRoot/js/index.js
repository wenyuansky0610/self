// =============================================================
	...
	var m3D = function() {
		/* ---- private vars ---- */
		var diapo = [], imb, scr, bar, selected, urlInfo, imagesPath = "images/", camera = {
			x : 0,
			y : 0,
			z : -650,
			s : 0,
		...
		var Diapo = function(n, img, x, y, z) {
			if (img) {
				...
				this.but.diapo = this;
				this.but.style.left = Math.round((this.but.offsetWidth * 1.2)
						* (n % 4))
						+ 'px';
				this.but.style.top = Math.round((this.but.offsetHeight * 1.2)
						* Math.floor(n / 4))
						+ 'px';
		...
		var interpolation = function(bicubic) {
			...
			while (o = diapo[i++]) {
				if (o.but) {
					o.css.msInterpolationMode = bicubic ? 'bicubic'
							: 'nearest-neighbor'; // makes IE8 happy
					o.css.imageRendering = bicubic ? 'optimizeQuality'
		...
		var run = function() {
			...
			if (camera.tx) {
				if (!camera.s)
					camera.setTarget(camera.x, camera.tx);
				...
				if (!camera.s)
					camera.setTarget(camera.y, camera.ty);
				...
				if (!camera.s)
					camera.setTarget(camera.z, camera.tz);
				...
				if (!m) {
					...
					if (selected.url) {
						...
						urlInfo.style.top = Math.round(selected.img.offsetTop
								+ selected.img.offsetHeight
								- urlInfo.offsetHeight - 5)
								+ "px";
						urlInfo.style.left = Math.round(selected.img.offsetLeft
								+ selected.img.offsetWidth
								- urlInfo.offsetWidth - 5)
								+ "px";
